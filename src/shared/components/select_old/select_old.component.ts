...
@Component({
  selector: 'app-select-old, p-select-old',
  templateUrl: './select_old.component.html',
  styleUrls: ['./select_old.component.css'],
  animations: [menuAnim],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => SelectOldComponent),
      multi: true,
    },
  ],
})
export class SelectOldComponent implements OnInit, OnDestroy, DoCheck {
  ...
  selectedOptions: SelectOptionOldComponent[] = [];
  multipleValues = [];

  optionButtons: SelectOptionOldComponent[] = [];

  allSelected = false;

  selectedTotal = 0;

  selectedOption: SelectOptionOldComponent;
  ...
  @ViewChildren(forwardRef(() => SelectOptionOldComponent))
  arrayGeneratedButtons: QueryList<SelectOptionOldComponent>;

  @ContentChildren(forwardRef(() => SelectOptionOldComponent), {
    descendants: true,
  })
  contentProjectionButtons: QueryList<SelectOptionOldComponent>;
...
@Component({
  selector: 'app-option-old, p-option-old',
  styleUrls: ['./select_old.component.css'],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => SelectOptionOldComponent),
      multi: true,
    },
  ],
  template: ` <button
...
export class SelectOptionOldComponent implements OnInit {
  @Input() value: any;
  @Input() disabled: boolean;

  selected: boolean;
  constructor(public parent: SelectOldComponent, public el: ElementRef) {}
...
export class SelectMultipleDirective implements OnInit {
  ...
  constructor(public parent: SelectOldComponent) {
